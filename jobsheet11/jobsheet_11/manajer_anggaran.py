...
class AnggaranHarian:
    ...
    def hapus_transaksi(self, id_transaksi: int) -> bool:
        """FITUR PENUGASAN: Menghapus data dari database berdasarkan ID"""
        query_hapus = "DELETE FROM transaksi WHERE id = ?"
        argumen = (id_transaksi,)
        
        # Eksekusi ke database dan kembalikan True jika sukses
        eksekusi = database.execute_query(query_hapus, argumen)
        return eksekusi is not None