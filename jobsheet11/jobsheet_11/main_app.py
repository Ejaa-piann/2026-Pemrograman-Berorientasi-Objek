...
def halaman_riwayat(anggaran: AnggaranHarian):
    ...
    if df_transaksi is None:
        ...
        st.dataframe(df_transaksi, use_container_width=True, hide_index=False)
        
        # --- POTONGAN KODE PENUGASAN (Sudah disinkronkan) ---
        st.markdown("---")
        st.subheader("⚙️ Panel Penghapusan Data")
        ...
        with zona_aksi:
            st.write("")  # Penyelaras vertikal agar tombol sejajar dengan input
            st.write("")
            eksekusi_hapus = st.button("Hapus Permanen", type="primary", use_container_width=True)
            
        if eksekusi_hapus:
            with st.spinner("Menghapus baris data..."):
                # PERBAIKAN: Menggunakan 'anggaran', bukan 'manajer_anggaran'
                apakah_terhapus = anggaran.hapus_transaksi(int(id_target))
                
                if apakah_terhapus:
                    st.toast(f"Sukses! ID {id_target} telah dihapus.", icon="🗑️")
                    st.cache_data.clear()  # Reset cache streamlit agar tabel langsung terupdate
